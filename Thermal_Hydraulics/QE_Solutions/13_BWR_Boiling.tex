...
\protect\caption{BWR Boiling Problem Illustration}
...
\begin{enumerate}
\item No Fast Fissions ($RR_{f}=\Sigma_{f}\phi_{T}$)
\item Steady State ($\frac{\partial}{\partial t}=0$)
\item No gravity ($\vec{g}=0$)
\item No Viscous Forces ($\boldsymbol{\tau}=0$)
\item Negligible pressure drop in coolant channel ($\nabla p=0$)
\item Negligible axial conduction ($k\frac{\partial^{2}T}{\partial z^{2}}=0$)
\end{enumerate}

\subsection{Power Density}

First, we must find the power density using the neutronics of the
reactor. We can find the flux in a cylindrical reactor by solving
for $\phi_{T}$ in
...
dimensions of the reactor, $\tilde{R}\approx R$ and $\tilde{H}\approx H$.
