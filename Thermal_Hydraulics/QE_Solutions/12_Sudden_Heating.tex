...
\begin{enumerate}
\item Axial Symmetry ($\frac{\partial}{\partial\theta}=0$)
\item Sufficiently small in radius ($\frac{\partial}{\partial r}=0$)
\item Well insulated at boundaries (no dissipation)
\item No heat generation in rod ($\dot{q}=0$)
\item Stationary (no M.E. or C.E., $\vec{v}=0$)
\item Irrotational (no R.M.E.)
\end{enumerate}
...
And we can express $\nicefrac{\theta}{\theta_{0}}$ in terms of a
non dimensional function including the three parameters ($t$, $x$,
and $\alpha$) in the original form. The $4$ is included to help
with the algebra.
